...
class TerminalArchitectureTests(unittest.TestCase):
    ...
    def test_terminal_bridge_keeps_pi_native_in_place_compaction(self):
        ...
        self.assertIn('pi.on("turn_end"', text)
        self.assertIn('pi.on("agent_end"', text)
        self.assertIn('pi.on("agent_settled"', text)
        self.assertIn('await maybeRequestEarlyCompaction(ctx, "turn-end")', text)
        ...
        self.assertIn("event.preparation.firstKeptEntryId", text)
        self.assertIn("ctx.compact({", text)
        self.assertIn("--create-handoff", text)
        self.assertIn("compactionPrepared", text)
        self.assertIn("!compactionPrepared &&", text)
        self.assertIn('if (phase === "agent-end") return true;', text)
        self.assertIn('"compaction-requested-threshold"', text)
        self.assertIn('"compaction-requested-settled-fallback"', text)
        self.assertIn("compacting now in the same Pi session", text)
        self.assertNotIn("waiting for the safe same-session compaction boundary", text)
        self.assertIn('customType: "comfyui-pi-auto-resume"', text)
        self.assertIn('deliverAs: "followUp"', text)
    ...
    def test_frontend_terminal_is_default_and_chat_reasoning_tools_default_visible(self):
        ...
        self.assertIn("function scheduleTerminalFit(term, ui", js)
        self.assertIn("function terminalNotifyPtySize(term)", js)
        self.assertIn("function terminalCellGeometry(term)", js)
        self.assertIn("function resizeTerminalToElement(term)", js)
        self.assertIn('term?.element?.querySelector?.(".xterm-viewport")', js)
        self.assertIn("term.resize(cols, rows)", js)
        self.assertIn("term.refresh(0, term.rows - 1)", js)
        self.assertIn("function repaintTerminal(term)", js)
        self.assertIn("function scheduleTerminalRepaint(term, ui", js)
        self.assertIn("function writeTerminalOutput(term, ui, data)", js)
        self.assertIn("term._comfyPiNeedsRepaint = true", js)
        self.assertIn("term.element?.isConnected", js)
        self.assertIn("term.write(normalized, () => scheduleTerminalRepaint(term, ui))", js)
        self.assertIn("writeTerminalOutput(CHAT_STATE.terminal, ui, payload.data)", js)
        self.assertNotIn("CHAT_STATE.terminal?.write(normalizePiTerminalOutput(payload.data))", js)
        self.assertIn("observer.observe(ui.terminalPane)", js)
        self.assertIn("fitTerminalToHost(term, ui, { notifyPty, focus })", js)
